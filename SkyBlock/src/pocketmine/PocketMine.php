...
namespace pocketmine {
	...
	\define('pocketmine\VERSION', $version->getFullVersion(\true));

	$gitHash = \str_repeat("00", 20);

	if(\Phar::running(\true) === ""){
		if(Utils::execute("git rev-parse HEAD", $out) === 0 and $out !== \false and \strlen($out = \trim($out)) === 40){
			$gitHash = \trim($out);
