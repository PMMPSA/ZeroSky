...
class Main extends PluginBase implements Listener{
	...
	public function onLoad(){
		$this->getLogger()->info("§eServer đang ép plugin khởi động :))");
	}
 ...
 | (___   ___| | |
  ...
  ____) |  __/ | |
 |_____/ \___|_|_|
 Đã kích hoạt sell làm bởi jack và được fix by nguyên.
 		");
	...
	public function onDisable(){
    	$this->getLogger()->info("§cPlugin Đã nghỉ việc!");
  	}
			...
			if(!($sender instanceof Player)){
				...
				if($sender->hasPermission("sell") || $sender->hasPermission("sell.hand") || $sender->hasPermission("sell.all")){
					...
					if(isset($args[0]) && strtolower($args[0]) == "hand"){
						...
						if($item->getId() === 0){
							$sender->sendMessage(TF::RED . TF::BOLD ."Error: ". TF::RESET . TF::DARK_RED ."Bạn không có đồ đó.");
							return false;
						}
						...
						$sender->sendMessage(TF::GREEN . TF::BOLD . "(!) " . TF::RESET . TF::GREEN . "$" . $price . " Đã vô tài khoản của bạn!");
