...
class Main extends PluginBase implements Listener{
	...
	public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
		if($sender instanceof Player){
			switch($command){
				case "/pos1":
					{
						if(!$sender->hasPermission("magicwe.command.pos1")) return false;
				...
				case "/pos2":
					{
						if(!$sender->hasPermission("magicwe.command.pos2")) return false;
				...
				case "/set":
					{
						if(!$sender->hasPermission("magicwe.command.set")) return false;
						if(isset($args[0])){
				...
				case "/replace":
					{
						if(!$sender->hasPermission("magicwe.command.replace")) return false;
				...
				case "/copy":
					{
						if(!$sender->hasPermission("magicwe.command.copy")) return false;
						if(isset($this->pos1[$sender->getName()], $this->pos2[$sender->getName()])){
							$this->copy($sender);
							return true;
						}
				...
				case "/paste":
					{
						if(!$sender->hasPermission("magicwe.command.paste")) return false;
						if(isset($this->pos1[$sender->getName()], $this->pos2[$sender->getName()])){
							$this->paste($sender);
							$sender->getLevel()->doChunkGarbageCollection();
							return true;
				...
				case "/undo":
					{
						if(!$sender->hasPermission("magicwe.command.undo")) return false;
				...
				case "/redo":
					{
						if(!$sender->hasPermission("magicwe.command.redo")) return false;
				...
				case "/flip":
					{
						if(!$sender->hasPermission("magicwe.command.flip")) return false;
				...
				case "/wand":
					{
						if(!$sender->hasPermission("magicwe.command.wand")) return false;
				...
				case "/schem":
					{
						if(!$sender->hasPermission("magicwe.command.schem")) return false;
				...
				case "/cyl":
					{
						if(!$sender->hasPermission("magicwe.command.cyl")) return false;
				...
				case "/hcyl":
					{
						if(!$sender->hasPermission("magicwe.command.hcyl")) return false;
						if(isset($args[0], $args[1])){
							#$this->fill($sender, $args[0]);
	...
	public function wandPos1(BlockBreakEvent $event){
		...
		if($sender->hasPermission("magicwe.command.wand") && $sender->getInventory()->getItemInHand()->getId() === Item::WOODEN_AXE && $this->wand[$sender->getName()] === 1){
			...
			$sender->sendMessage(TextFormat::GREEN . "[MagicWE] Position 1 set as x:" . $block->x . " y:" . $block->y . " z:" . $block->z." Id: {$block->getId()}:{$block->getDamage()}");
			$event->setCancelled();
		}
	}
	...
	public function wandPos2(PlayerInteractEvent $event){
		...
		if($sender->hasPermission("magicwe.command.wand") && $sender->getInventory()->getItemInHand()->getId() === Item::WOODEN_AXE && $this->wand[$sender->getName()] === 1){
			...
			$sender->sendMessage(TextFormat::GREEN . "[MagicWE] Position 2 set as x:" . $block->x . " y:" . $block->y . " z:" . $block->z." Id: {$block->getId()}:{$block->getDamage()}");
			$event->setCancelled();
		}
	}
	...
	public function undo(Player $player){
		$time = microtime(TRUE);
		$level = $player->getLevel();
		if(!isset($this->undo[$player->getName()])) return false;
	...
	public function redo(Player $player){
		$time = microtime(TRUE);
		$level = $player->getLevel();
		if(!isset($this->redo[$player->getName()])) return false;
