	...
	public function factionChat(PlayerChatEvent $PCE) {
		...
		if($this->plugin->motdWaiting($player)) {
			if(time() - $this->plugin->getMOTDTime($player) > 30) {
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("Timed out. Please use /g desc again."));
				...
				$PCE->setCancelled(true);
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("Successfully updated the faction description. Type /g info.", true));
			}
			return true;
		}
	...
	public function factionBlockBreakProtect(BlockBreakEvent $event) {
		...
		if($this->plugin->pointIsInPlot($x, $z, $level)){
			if($this->plugin->factionFromPoint($x, $z, $level) === $this->plugin->getFaction($event->getPlayer()->getName())){
				...
				$event->getPlayer()->sendMessage($this->plugin->formatMessage("You cannot break blocks here. This is already a property of a faction. Type /g plotinfo for details."));
				return;
			}
		}
	}
	...
	public function factionBlockPlaceProtect(BlockPlaceEvent $event) {
		...
		if($this->plugin->pointIsInPlot($x, $z, $level)) {
			if($this->plugin->factionFromPoint($x, $z, $level) == $this->plugin->getFaction($event->getPlayer()->getName())) {
				...
				$event->setCancelled(true);
				$event->getPlayer()->sendMessage($this->plugin->formatMessage("You cannot place blocks here. This is already a property of a faction. Type /g plotinfo for details."));
				return;
			}
		}
	}
