            ...
            switch ($args[0]) {
                case "about":
                    if (!$sender->hasPermission("piggycustomenchants.command.ce.about")) {
                        $this->error($sender, TextFormat::RED . "Bạn không có quyền để sử dụng CustomEnchants!");
                        return false;
                    }
                    if ($forms) {
                    ...
                    $sender->sendMessage(TextFormat::GREEN . "CustomEnchants v" . $this->getPlugin()->getDescription()->getVersion() . "  được tạo bởi DaPigGuy" . TextFormat::GREEN . " và được fix bởi Thanh Nguyên");
                    break;
                case "enchant":
                    ...
                    if (count($args) < 2) {
                        ...
                        $sender->sendMessage(TextFormat::RED . "Usage: /customenchant enchant <enchant> [cấp độ] [Tên người chơi]");
                        return false;
                    }
                    if ($forms) {
                ...
                case "help":
                    ...
                    $sender->sendMessage(TextFormat::GREEN . "---Trợ giúp CE---\n" . TextFormat::RESET . "/ce about: Thông tin về người tạo ra plugin\n/ce enchant: Phù phép\n/ce help: Xem bảng hỗ trợ\n/ce info: Xem tác dụng của loại phù phép đó\n/ce list: Các loại phù phép");
                    break;
                case "info":
    ...
    public function aboutForm(Player $player)
    {
        ...
        if ($plugin instanceof Main) {
            ...
            if ($plugin->formsEnabled && $formsapi instanceof \jojoe77777\FormAPI\FormAPI && $formsapi->isEnabled()) {
                ...
                $form->setContent(TextFormat::GREEN . "CustomEnchants v" . $this->getPlugin()->getDescription()->getVersion() . "  được tạo bởi DaPigGuy" . TextFormat::GREEN . "và được fix bởi Thanh Nguyên");
                $form->addButton(TextFormat::BOLD . "Back");
                $form->sendToPlayer($player);
            }
        }
