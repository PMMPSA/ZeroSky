...
class Main extends PluginBase
{
    ...
    public function onEnable()
    {
        if (!$this->isSpoon()) {
            ...
            $this->getLogger()->info(TextFormat::GREEN . "Đã kiểm tra plugin và chạy!");
        }
    }
    ...
    {
        if ($this->getDescription()->getAuthors() !== ["ThanhNguyen(Suphuhugodzgamer)"] || $this->getDescription()->getName() !== "CustomEnchants") {
            $this->getLogger()->error("Nếu đổi author và tên plugin thì đừng hòng xài :>");
            return true;
        }
        return false;
    }
