...
public function onCommand(CommandSender $s,Command $cmd,string $label, array $args) : bool{
			...
			if(isset($args[0]) && isset($args[1])) {				  
				if(is_numeric($args[0])) {					  
					if(is_numeric($args[1])) {						
						if($this->eco->myMoney($s->getName()) >= "7000"*$args[1]) {		  
							$enchantLevel = $args[1];
							$enchantId = $args[0];
							...
							*/
							...
							$this->eco->reduceMoney($s->getName(), "7000"*$args[1]);
		...
		if($cmd->getName() == "tokenenchant") {
			
			if(isset($args[0]) && isset($args[1])) {				  
				if(is_numeric($args[0])) {					  
					if(is_numeric($args[1])) {						
						if($this->eco->myItem($s->getName()) >= "5.0.0") {		  
							$enchantLevel = $args[1];
							$enchantId = $args[0];
							$enchantment = Enchantment::getEnchantment($enchantId);
							...
							*/
							...
							$this->eco->reduceItem($s->getName(), "5.0.0");
				...
				$s->sendMessage("sử dụng: /tokenenchant <ID> <cấp độ>");
				return false;
			}
		}
